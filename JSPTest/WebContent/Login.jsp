...
<body>
	...
	try {
		...
		if(check == false){
			out.println("<script>alert('고객의 정보가 없습니다.'); location.href='login.jsp'</script>"); // 만약에 rs 가 한번도 안돌았으면
		}
		else if(check == true){
			out.println("<script>alert('success login'); location.href='success_login.jsp'</script>");// 만약에 rs 가 한번 돌았으면 그 값이 있다는 뜻
		}
