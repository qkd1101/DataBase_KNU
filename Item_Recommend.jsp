...
<body>
	...
	<%
		...
		System.out.println(age + " 1 " + sex + " 2 ");
		Connection con = null;
		ResultSet rs = null;
		if (age != null && sex != null) {
			...
			String query = "create or replace view rec as select sh.i_id as id, count(*) as num from shoppingbag sb, ordered_shop_has sh where sb.order_date is not null and sb.shop_id = sh.s_id and sb.cus_no in (select cus_id from customer where sex = '"
					+ sex + "' and age between " + age + "-5 and " + age
					+ "+5) group by sh.i_id;";
	...
	<%
		System.out.println("come in2");
			...
			String query = "create or replace view rec as select sh.i_id as id, count(*) as num from shoppingbag sb, ordered_shop_has sh where sb.order_date is not null and sb.shop_id = sh.s_id and sb.cus_no in (select cus_id from customer where sex = '"
					+ sex + "' and age is null) group by sh.i_id;";
			System.out.println(query);
			try {
				String url = "jdbc:mysql://localhost:3306/test";
				Class.forName("com.mysql.jdbc.Driver").newInstance();
	...
	<%
		System.out.println("come in2");
			...
			String query = "create or replace view rec as select sh.i_id as id, count(*) as num from shoppingbag sb, ordered_shop_has sh where sb.order_date is not null and sb.shop_id = sh.s_id and sb.cus_no in (select cus_id from customer where sex is null and age between " + age + "-5 and " + age
					+ "+5) group by sh.i_id;";
			System.out.println(query);
			try {
				String url = "jdbc:mysql://localhost:3306/test";
	...
	<%
		System.out.println("come in2");
			...
			String query = "create or replace view rec as select sh.i_id as id, count(*) as num from shoppingbag sb, ordered_shop_has sh where sb.order_date is not null and sb.shop_id = sh.s_id and sb.cus_no in (select cus_id from customer where sex is null and age is null) group by sh.i_id;";
			System.out.println(query);
			try {
				String url = "jdbc:mysql://localhost:3306/test";
				Class.forName("com.mysql.jdbc.Driver").newInstance();
	...
	%>
	<a href="MainMenu_Main.jsp">메인 메뉴로 돌아가기</a>&nbsp;&nbsp;&nbsp;
