	...
	<%
		...
		if (check) { //재고가 충분할 경우
				...
				while (rs.next()) {
					...
					try {
						...
						if (rs2.next()) //재고가 존재할 경우
						{
							...
							try { //매장의 재고를 줄임
								String query3 = "update ret_has set stock = stock - " + quantity
										+ " where item_id = '" + i_id + "' and ret_id = '" + ret_id + "';"; //매장의 재고를 줄임.
							...
							try { //상품의 재고를 줄임
								String query3 = "update item set stock = stock - " + quantity + " where item_id = '"
										+ i_id + "';"; //상품의 재고를 줄임.
						...
						else //재고가 부족할 경우
						{
							try {
								...
								if (!rs3.next()) {
									out.println(
											"<script>alert('재고가 부족합니다.'); location.href='Shoppingbag.jsp'</script>");
								}

								String ret_id = rs3.getString(1); //재고가 존재하는 매장의 아이디 저장

								try { //매장의 재고를 줄임
									String query4 = "update ret_has set stock = stock - " + quantity
											+ "where item_id = '" + i_id + "' and ret_id = '" + ret_id + "';"; //매장의 재고를 줄임.
								...
								try { //상품의 재고를 줄임
									String query4 = "update item set stock = stock - " + quantity
											+ "where item_id = '" + i_id + "';"; //상품의 재고를 줄임.
			...
			try {
				...
				try {
					...
					while (result.next()) {
						...
						String query2 = "insert into ordered_shop_has values('" + s_id + "', '" + i_id + "', "
								+ i_quantity + ");";
			...
			try {
				...
				query = "update shoppingbag set order_date = '" + year + "-" + month + "-" + date
						+ "' where cus_no = '" + id + "' and order_date is null;";
				System.out.println(query);
				PreparedStatement pstmt = con.prepareStatement(query);
				pstmt.executeUpdate(query);
			...
			//고객의 주문 횟수 증가
			try {
				query = "update customer set ordertime = ordertime + 1 where cus_id = '" + id + "';";
				System.out.println(query);
				PreparedStatement pstmt = con.prepareStatement(query);
				pstmt.executeUpdate(query);
			}
